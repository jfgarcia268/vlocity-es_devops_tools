...
- Please read carefully the documentation and check with the EPC implemtation team that this fits your needs and the match Scenarios do apply for your implementation, Used it at your own risk. 

Sync Global Keys between two orgs (from source to target org) for the Following objects:

#### Attribute Assignments Scenarios:

##### 1. Non Override AA (IsOverride__c = false )
...
##### 2. Override AA (IsOverride__c = true )

This will query All the OverideDefinitions by using the considitions:
...
USAGE
  ...
  $ sfdx vlocityestools:clean:epcgkfix -s <string> -t <string>  [-p cmt] [-c] [-a]
